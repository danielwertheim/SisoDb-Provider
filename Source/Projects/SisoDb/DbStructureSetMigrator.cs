...
namespace SisoDb
{
	public class DbStructureSetMigrator : IStructureSetMigrator
	{
			...
			where TNew : class
		{
			...
			using (var dbClientTransactional = Db.ProviderFactory.GetTransactionalDbClient(Db.ConnectionInfo))
			{
				try
				{
				...
				finally
				{
					...
					{
						Db.StructureSchemas.RemoveSchema(oldType);
						Db.SchemaManager.DropStructureSet(structureSchemaOld, dbClientTransactional);
					}
				}
			}
		}
			...
			where TNew : class
		{
			...
			using (var dbClientNonTrans = Db.ProviderFactory.GetNonTransactionalDbClient(Db.ConnectionInfo))
			{
				foreach (var json in dbClientNonTrans.GetJsonOrderedByStructureId(structureSchemaOld))
				{

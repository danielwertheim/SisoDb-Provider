...
{
    internal class LambdaParser : ExpressionVisitor, ILambdaParser
    {
        ...
        public IParsedLambda Parse<T>(Expression<Func<T, bool>> e)
        {
            ...
            lock (_lock)
            {
                _nodesContainer = new NodesContainer(e);

                Visit(e);

                return new ParsedLambda(_nodesContainer.Expression, _nodesContainer.Nodes);
            }
        }
        ...
        protected override Expression VisitConstant(ConstantExpression e)
        {
            ...
            {
                if (!(value is string))
                    value = StringConverter.AsString(value);

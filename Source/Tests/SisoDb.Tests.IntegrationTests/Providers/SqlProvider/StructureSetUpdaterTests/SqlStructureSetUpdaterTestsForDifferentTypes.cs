...
{
    [TestFixture]
    public class SqlStructureSetUpdaterTestsForDifferentTypes : SqlIntegrationTestBase
    {
        ...
        public void Process_WhenDifferentObjectWherePropertiesAreRemapped_FancyResult()
        {
            ...
            using(var uow = Database.CreateUnitOfWork())
            {
                var refetchedPerson = uow.GetById<Person>(orgItem.Id);
                Assert.IsNull(refetchedPerson);

                var refetchedSalesPerson = uow.GetById<SalesPerson>(orgItem.Id);

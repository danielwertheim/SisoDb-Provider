...
{
    [TestFixture]
    public class SqlUnitOfWorkWhereStringUsingQxStartsWithTests : IntegrationTestBase
    {
        ...
        public void QueryStringUsingQxStartsWith_NoMatch_NullIsReturned()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                refetched = uow.Where<QxItemForQueries>(i => i.StringValue.QxStartsWith("C")).SingleOrDefault();
            }
        ...
        public void QueryStringUsingQxStartsWith_MatchingStart_ItemIsReturned()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                refetched = uow.Where<QxItemForQueries>(i => i.StringValue.QxStartsWith("AB")).SingleOrDefault();
            }
        ...
        public void QueryStringUsingQxStartsWith_CompleteMatch_ItemIsReturned()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                refetched = uow.Where<QxItemForQueries>(i => i.StringValue.QxStartsWith("ABC")).SingleOrDefault();
            }

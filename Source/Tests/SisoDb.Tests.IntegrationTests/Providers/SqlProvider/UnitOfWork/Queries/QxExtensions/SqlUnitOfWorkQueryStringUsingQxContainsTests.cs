...
{
    [TestFixture]
    public class SqlUnitOfWorkQueryStringUsingQxContainsTests : IntegrationTestBase
    {
        ...
        public void QueryStringUsingQxContains_MatchInMiddleOfStringExists_ItemIsReturned()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                refetched = uow.Query<QxItemForQueries>(i => i.StringValue.QxContains("B")).SingleOrDefault();
            }
        ...
        public void QueryStringUsingQxContains_MatchInEndOfStringExists_ItemIsReturned()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                refetched = uow.Query<QxItemForQueries>(i => i.StringValue.QxContains("C")).SingleOrDefault();
            }

...
{
    [TestFixture]
    public class Sql2008UnitOfWorkQueryTests : Sql2008IntegrationTestBase
    {
        ...
        public void Query_ByUsingInsertedItemInExpression_ReturnsMatchingItem()
        {
            ...
            using (var uow = Database.CreateUnitOfWork())
            {
                ...
                refetched = uow.Query<SimpleItemForQueries>(
                    q => q.Where(x => x.SortOrder == item.SortOrder)).SingleOrDefault();
            }
        ...
        public void QueryIntByInterval_WhenInRange_ReturnsMatchingItems()
        {
            ...
            {
                refetched = unitOfWork.Query<SimpleItemForQueries>(
                    q => q.Where(i => i.SortOrder >= 3 && i.SortOrder <= 6)).ToList();
            }
        ...
        public void Query_WhenQueryYieldsNoResult_NonNullEmptyResult()
        {
            ...
            {
                refetched = unitOfWork.Query<SimpleItemForQueries>(
                    q => q.Where(i => i.SortOrder < 1)).ToList();
            }
        ...
        public void QueryAsJsonIntByInterval_WhenInRange_ReturnsMatchingItems()
        {
            ...
            using (var unitOfWork = Database.CreateUnitOfWork())
            {
                refetched = unitOfWork.QueryAsJson<SimpleItemForQueries>(
                    q => q.Where(i => i.SortOrder >= 3 && i.SortOrder <= 6)).ToList();
            }

            Assert.AreEqual(4, refetched.Count());
            Assert.AreEqual("{\"StructureId\":3,\"SortOrder\":3}", refetched[0]);
            Assert.AreEqual("{\"StructureId\":6,\"SortOrder\":6}", refetched[3]);
        }
        ...
        public void QueryAs_StoredAsXReturnAsY_ItemReturnedGetsPopulated()
        {
            ...
            using (var unitOfWork = Database.CreateUnitOfWork())
            {
                refetched = unitOfWork.QueryAs<SimpleItemForQueries, SimpleItemForQueriesInfo>(
                    q => q.Where(i => i.StructureId != 0).SortBy(c => c.SortOrder)).ToList();
            }

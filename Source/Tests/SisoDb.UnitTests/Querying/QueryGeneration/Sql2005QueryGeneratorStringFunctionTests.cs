...
namespace SisoDb.UnitTests.Querying.QueryGeneration
{
    ...
    public class Sql2005QueryGeneratorStringFunctionTests : QueryGeneratorStringFunctionTests
    {
        ...
        public override void GenerateQuery_for_Where_with_Text_QxIsExactly_GeneratesCorrectQuery()
        {
            ...
            Assert.AreEqual(
                "select s.[Json] from (select s.[StructureId] from [MyClassStructure] s left join [MyClassTexts] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'MyText' where ((mem0.[Value] = @p0) and (cast(mem0.[Value] as varbinary) = cast(@p0 as varbinary))) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
                sqlQuery.Sql);

...
namespace SisoDb.UnitTests.Querying.QueryGeneration
{
    ...
    public class Sql2012QueryGeneratorStringFunctionTests : QueryGeneratorStringFunctionTests
    {
    	...
    	public override void GenerateQuery_for_Where_with_String_StartsWith_on_Nullable_ToString_GeneratesCorrectQuery()
    	{
			...
			Assert.AreEqual(
				"select s.[Json] from (select s.[StructureId] from [MyClassStructure] s inner join [MyClassIntegers] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'NullableInt1' where (mem0.[StringValue] like @p0) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
				sqlQuery.Sql);
    	...
    	public override void GenerateQuery_for_Where_with_String_Equals_and_StartsWith_on_String_GeneratesCorrectQuery()
    	{
			...
			Assert.AreEqual(
				"select s.[Json] from (select s.[StructureId] from [MyClassStructure] s inner join [MyClassStrings] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'String1' where ((mem0.[Value] = @p0) or (mem0.[Value] like @p1)) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
				sqlQuery.Sql);
        ...
        public override void GenerateQuery_for_Where_with_String_StartsWith_GeneratesCorrectQuery()
        {
            ...
            Assert.AreEqual(
				"select s.[Json] from (select s.[StructureId] from [MyClassStructure] s inner join [MyClassStrings] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'String1' where (mem0.[Value] like @p0) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
                sqlQuery.Sql);
        ...
        public override void GenerateQuery_for_Where_with_String_ToLower_GeneratesCorrectQuery()
        {
            ...
            Assert.AreEqual(
				"select s.[Json] from (select s.[StructureId] from [MyClassStructure] s inner join [MyClassStrings] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'String1' where (lower(mem0.[Value]) = @p0) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
                sqlQuery.Sql);
        ...
        public override void GenerateQuery_for_Where_with_String_ToUpper_GeneratesCorrectQuery()
        {
            ...
            Assert.AreEqual(
				"select s.[Json] from (select s.[StructureId] from [MyClassStructure] s inner join [MyClassStrings] mem0 on mem0.[StructureId] = s.[StructureId] and mem0.[MemberPath] = 'String1' where (upper(mem0.[Value]) = @p0) group by s.[StructureId]) rs inner join [MyClassStructure] s on s.[StructureId] = rs.[StructureId];",
                sqlQuery.Sql);
